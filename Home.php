...
$tempUser = $adrlist->query("SELECT * FROM `adress` WHERE `addr`='$ip'");
$user = $tempUser->fetch_assoc();
$counter = $user['count'] + 1;
if ($user === NULL) {
    $adrlist->query("INSERT INTO `adress` (`addr`, `count`) VALUES('$ip', '1')");
} else {
    $adrlist->query("UPDATE `adress` SET `count`='$counter' WHERE `addr`='$ip'");
}
$adrlist->close();
?>
<body class="bodyHome">
